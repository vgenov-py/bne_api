...
def get_res(url, message=False):
    # print(req.get(url).json()["data"])
    if message:
        return req.get(url).json()["message"]
...
class Test_routes(unittest.TestCase):
    ...
    def test_errors(self):
        url =f"http://127.0.0.1:3000/api/geo?"
        self.assertEqual(get_res(f"{url}limit=s", message=True), "Limit value should be an integer")
        self.assertEqual(get_res(f"{url}limit=e", message=True), "Limit value should be an integer")
        self.assertEqual(get_res(f"{url}limit=i", message=True), "Limit value should be an integer")
